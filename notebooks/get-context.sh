...
progress_file="progress-2.log"
summary_file="autoregressive-cognition.txt"
main_dir=$(pwd)
...
process_files() {
    ...
    for file in "$dir"/*.txt; do
        ...
        # Skip processing the content file
        if [ "$(basename "$file")" == "content.txt" ]; then
            echo "Skipping content file: content.txt"
            continue
        fi
